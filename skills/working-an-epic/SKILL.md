...
When no child is left actionable, stop and report the whole stack with its merge order. `item_get`
the epic; if it has a `parent`, `item_get` that too, and if its title starts `Spec:`, end the report
with:
